describe('ConfigurationBasicQueryGerman', function () {
  var queryEngine;

  beforeEach(function () {
    queryEngine = new bur.ConfigurationBasicQueryGerman(testHelpers.vehicle);
  });
  ...
  describe('querying', function () {
    ...
    it('should return a clone of vehicle 123 with color blue querying a vehicle with color blue and configuration matching msc 221', function(){
      var newColor = 'blue',
          newConfig = queryEngine.getConfigurationWith(newColor, 'color', testHelpers.vehicle.mscs[7]);

      expect(newConfig.msc).toEqual('123');
      expect(newConfig.color).toEqual(newColor);
    });

    it('should return a clone of vehicle 111 querying a wehicle with grade basic', function(){
      var newConfig = queryEngine.getConfigurationWith('basic', 'grade', testHelpers.vehicle.mscs[4]);
      expect(newConfig.msc).toEqual('111');
    });
  });
});
